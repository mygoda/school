...
class UpyunFileImageWidget(ClearableFileInput):
    def render(self, name, value, attrs=None):
        template = super(UpyunFileImageWidget, self).render(name, value, attrs)
        append = '<p>%s</p><a href="%s" target="_blank"><img src="%s" width="100" height="100"></a>' % (value, value, value)
        return mark_safe(template + append)